...
def process_ocr_job(image_id):
    try:
        with transaction.atomic():
            ...
            logger.info('image_id=%s path=%s', image_id, path)
            if not os.path.exists(path):
                raise RuntimeError(f'image_id {image_id}: File not found: {path}')

            with open(path, 'rb') as handle:
                data = handle.read()

            # Data now has the image. logger.info('image_id %s: data %s', image_id, data)
            ...
            if application_id is None:
                raise RuntimeError(f'image_id {image_id}: Missing application id')
